...
if __name__ == "__main__":
    ...
    # Load time series and attribute data
    CO_timeseries_path = "../data/raw/CO_timeseries.csv"  # Or prediction raw data path with no label column
    CO_processed_path = (
        "../data/raw/CO_processed.csv"  # And corresponding prediction raw data path
    )
    ...
    # Fill data that has NA's with -1, to avoid droppin last year's products
    ...
    data["fecha_de_visita_dt"] = pd.to_datetime(
        data["fecha_de_visita"], format="%Y-%m-%d"
    )
    ...
    train.to_csv(
        CO_timeseries_model_data_prepath + "train_split_format.csv", index=False
    )
    ...
    # Save heldout form
    train = data[
        data["fecha_de_visita_dt"]
        < pd.to_datetime(heldout_threshold, format="%Y-%m-%d")
    ]
    test = data[
        data["fecha_de_visita_dt"]
        >= pd.to_datetime(heldout_threshold, format="%Y-%m-%d")
    ...
    train.to_csv(
        CO_timeseries_model_data_prepath + "train_heldout_format.csv", index=False
    )
    print("Saving Test Heldout Format")
    test.to_csv(
        CO_timeseries_model_data_prepath + "test_heldout_format.csv", index=False
    )
