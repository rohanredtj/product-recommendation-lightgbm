...
if __name__ == "__main__":
    ...
    baseline = round(
        max(bought_count, not_bought_count) / (bought_count + not_bought_count), 2
    )
    ...
    prediction_table = test[
        ...
        ["codigo_de_cliente", "fecha_de_visita", "prediction probability"],
        ascending=False,
    )
    prediction_table["rank"] = 1
    prediction_table["rank"] = prediction_table.groupby(
        ["codigo_de_cliente", "fecha_de_visita"]
    )["rank"].cumsum()
    ...
    prediction_table["codigo_de_cliente"] = prediction_table.codigo_de_cliente.astype(
        str
    )
    prediction_table["codigo_de_producto"] = prediction_table.codigo_de_producto.astype(
        str
    )
    prediction_table = pd.merge(
        ...
        df_recom,
        how="left",
        left_on=["codigo_de_cliente", "fecha_de_visita"],
        right_on=["codigo_de_cliente", "fecha_de_visita"],
    )
    prediction_table = prediction_table[
        prediction_table["rank"] <= prediction_table["recomm_qt"]
    ]
    prediction_table = pd.merge(
        prediction_table,
        df_sales,
        how="left",
        left_on=["codigo_de_cliente", "fecha_de_visita", "codigo_de_producto"],
        right_on=["codigo_de_cliente", "fecha_de_visita", "codigo_de_producto"],
    )

    prediction_table["venta_neta_dolar"] = prediction_table["venta_neta_dolar"].fillna(
        0.00
    )
    prediction_table["executed_flag"] = prediction_table["venta_neta_dolar"].apply(
        lambda x: 1 if x > 0.00 else 0
    )
    ...
    precision = round(
        temp[temp["bought_in_the_visit"] == 1].shape[0] / temp.shape[0], 4
    )
    print("\nPrecision: for class 1 %.2f%%" % (precision * 100.0), file=report_object)

    temp = prediction_table[prediction_table["predictions"] == 0]
    precision = round(
        temp[temp["bought_in_the_visit"] == 0].shape[0] / temp.shape[0], 4
    )
    print("\nPrecision: for class 0 %.2f%%" % (precision * 100.0), file=report_object)

    client_level_precision = (
        prediction_table.groupby(["codigo_de_cliente"])
        .agg({"codigo_de_producto": "count", "executed_flag": "sum"})
        .reset_index()
    )
    client_level_precision["precision"] = (
        client_level_precision["executed_flag"]
        / client_level_precision["codigo_de_producto"]
    )

    client_and_visit_level_precision = (
        prediction_table.groupby(["fecha_de_visita", "codigo_de_cliente"])
        .agg({"codigo_de_producto": "count", "executed_flag": "sum"})
        .reset_index()
    )
    client_and_visit_level_precision["non_executed_flag"] = (
        client_and_visit_level_precision["codigo_de_producto"]
        - client_and_visit_level_precision["executed_flag"]
    )
    client_and_visit_level_precision["precision_execution_flag"] = (
        client_and_visit_level_precision["executed_flag"]
        / client_and_visit_level_precision["codigo_de_producto"]
    )
    client_and_visit_level_precision["precision_non_execution_flag"] = (
        client_and_visit_level_precision["non_executed_flag"]
        / client_and_visit_level_precision["codigo_de_producto"]
    )

    precision_class_1 = client_and_visit_level_precision[
        "precision_execution_flag"
    ].mean()
    precision_class_2 = client_and_visit_level_precision[
        "precision_non_execution_flag"
    ].mean()

    client_level_precision.to_csv(
        precision_report_prepath + exp_id + "_client_level_precision.csv", index=False
    )
    client_and_visit_level_precision.to_csv(
        precision_report_prepath + exp_id + "_client_and_visit_level_precision.csv",
        index=False,
    )
    print(
        "\nClient and Visit Level Mean precision for class 1:",
        round(precision_class_1, 2),
        file=report_object,
    )
    print(
        "\nClient and Visit Level Mean precision for class 0:",
        round(precision_class_2, 2),
        file=report_object,
    )
    ...
    for train_idx, val_idx in kfold.split(full):
        ...
        prediction_table["fecha_de_visita"] = prediction_table.fecha_de_visita.astype(
            str
        )
        prediction_table[
            "codigo_de_cliente"
        ] = prediction_table.codigo_de_cliente.astype(str)
        prediction_table[
            "codigo_de_producto"
        ] = prediction_table.codigo_de_producto.astype(str)
        prediction_table = pd.merge(
            prediction_table,
            df_recom,
            how="left",
        ...
        prediction_table["venta_neta_dolar"] = prediction_table[
            "venta_neta_dolar"
        ].fillna(0.00)
        prediction_table["executed_flag"] = prediction_table["venta_neta_dolar"].apply(
            lambda x: 1 if x > 0.00 else 0
        ...
        overall_precision_after_trimming = precision_score(
            prediction_table["bought_in_the_visit"], prediction_table["predictions"]
        )

        overall_precision_after_trimming_scores.append(
            round(overall_precision_after_trimming, 2)
        )

    print("CV accuracy each fold:", acc_scores, file=report_object)
    print(
        "CV accuracy Mean:", round(np.mean(np.array(acc_scores)), 2), file=report_object
    )

    print("CV recall each fold:", rec_scores, file=report_object)
    print(
        "CV recall Mean:", round(np.mean(np.array(rec_scores)), 2), file=report_object
    )

    print("CV precision each fold:", pre_scores, file=report_object)
    print(
        "CV precision Mean:",
        round(np.mean(np.array(pre_scores)), 2),
        file=report_object,
    )
    ...
    print(
        "CV overall_precision_after_trimming each fold:",
        overall_precision_after_trimming_scores,
        file=report_object,
    )
    print(
        "CV overall_precision_after_trimming Mean:",
        round(np.mean(np.array(overall_precision_after_trimming_scores)), 2),
        file=report_object,
    )
