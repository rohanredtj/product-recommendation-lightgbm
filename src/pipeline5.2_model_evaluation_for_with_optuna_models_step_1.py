...
if __name__ == '__main__':
    ...
    exp_id = '20210817v2_lgb_with_optuna_on_cv_overall_precision_after_trimming_on_full_data_normalized_weeks_without_tsfresh_features_and_categorical_original_saved_model_on_90perc_split_data'
    ...
    ############################################################
    print('\n### split format (LGB)')
    print('\n### split format (LGB)', file=report_object)
    ...
    client_level_precision.to_csv(report_prepath+ exp_id + '_client_level_precision.csv', index=False)
    client_and_visit_level_precision.to_csv(report_prepath+ exp_id + '_client_and_visit_level_precision.csv', index=False)
    print('\nClient and Visit Level Mean precision ', precision)
    print('\nClient and Visit Level Mean precision', precision, file=report_object)
...
#     CV Evaluation
    ...
    for train_idx, val_idx in kfold.split(full):
        ...
        acc_scores.append(accuracy_score(y_val, predictions))
        rec_scores.append(recall_score(y_val, predictions))
        pre_scores.append(precision_score(y_val, predictions))
        ...
        overall_precision_after_trimming_scores.append(overall_precision_after_trimming)
